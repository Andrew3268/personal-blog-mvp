...
export async function onRequestGet({ env }) {
  const rows = await env.BLOG_DB
    .prepare(`
      SELECT slug, title, category, summary, cover_image, template_name, tags_json, status, published_at, updated_at
      FROM posts
...
export async function onRequestPost({ env, request }) {
  ...
  if (!slug || !title || !contentMd) {
    return okJson({ message: "slug, title, content_md는 필수입니다." }, { status: 400 });
  }
