...
export function renderInlineImageHtml(data = {}, options = {}) {
  ...
  if (!imageId) return "";
  const deliveryHash = String(options.deliveryHash || "").trim();
  if (!deliveryHash) return "";
  const variant = String(options.variant || "post-inline").trim() || "post-inline";
  const alt = String(data.alt || "본문 이미지").trim();
  const caption = String(data.caption || "").trim();
  const src = `https://imagedelivery.net/${deliveryHash}/${encodeURIComponent(imageId)}/${variant}`;
  return `
    <figure class="post-inline-image">
      <img class="post-inline-image__img" src="${escapeHtml(src)}" alt="${escapeHtml(alt)}" loading="lazy" decoding="async" />
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  const deliveryHash = String(options.deliveryHash || "").trim();
  const imageVariant = String(options.imageVariant || "post-inline").trim() || "post-inline";
  let h2Count = 0;
    ...
    if (heading) {
      ...
      if (level === 2) {
        ...
        if (h2Count === 3 && inlineImages.image1?.enabled && inlineImages.image1?.id) {
          pushBlock("inline-image", renderInlineImageHtml(inlineImages.image1, { deliveryHash, variant: imageVariant }));
        }
        if (h2Count === 5 && inlineImages.image2?.enabled && inlineImages.image2?.id) {
          pushBlock("inline-image", renderInlineImageHtml(inlineImages.image2, { deliveryHash, variant: imageVariant }));
        }
      }
      continue;
    }
