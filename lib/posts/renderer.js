...
import { parsePostLinkStyle, stripPostLinkStyleTokens } from "./link-style.js";

export { parsePostLinkStyle, stripPostLinkStyleTokens } from "./link-style.js";
...
export function renderInlineImageHtml(data = {}, options = {}) {
  ...
  return `
    <figure class="post-inline-image">
      <img class="post-inline-image__img" ${image.attrs} alt="${escapeHtml(alt)}" loading="lazy" decoding="async" referrerpolicy="no-referrer" width="1200" height="675" style="aspect-ratio:auto 16 / 9" />
  ...
  return `
    <article class="post-affiliate-card">
      <div class="post-affiliate-card__media">
        ${affiliateImage ? `<img class="post-affiliate-card__img" ${affiliateImage.attrs} alt="${escapeHtml(data.productName || "제휴 상품")}" loading="lazy" decoding="async" referrerpolicy="no-referrer" width="480" height="480" style="aspect-ratio:auto 1 / 1" />` : `<div class="post-affiliate-card__placeholder">상품 이미지</div>`}
      </div>
...
function inlineFormat(text = "") {
  ...
  let html = escapeHtml(protectedText)
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/`([^`]+)`/g, "<code>$1</code>")
    .replace(/\[([^\]]+)\]\((https?:[^\s)]+)\)/g, '<a class="post-content-link" href="$2" target="_blank" rel="noopener noreferrer">$1</a>')
    .replace(/\[([^\]]+)\]\((\/[^\s)]+)\)/g, '<a class="post-content-link" href="$2">$1</a>');
...
export function renderMarkdownBlocks(md = "", options = {}) {
  const normalizedMd = stripPostLinkStyleTokens(stripLsiKeywordsTokenLines(String(md || "")));
  ...
  function appendContinuationToOpenList(text) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!trimmed) {
      flushParagraph();
      const nextTrimmed = nextNonEmptyLine(index + 1);
      if (orderedItems.length && /^\d+\.\s+/.test(nextTrimmed)) {
        flushList();
        flushBlockquote();
        continue;
      }
      if (listItems.length && /^[-*]\s+/.test(nextTrimmed)) {
        flushOrdered();
        flushBlockquote();
        continue;
      }
    ...
    if (appendContinuationToOpenList(trimmed)) {
      continue;
    }
