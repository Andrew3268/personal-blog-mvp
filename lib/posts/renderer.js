...
function renderLinkButtonHtml(data = {}) {
...
export function renderMarkdownBlocks(md = "", options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const linkButton = parseLinkButtonToken(trimmed);
    if (linkButton) {
      flushParagraph();
      flushList();
      flushOrdered();
      flushBlockquote();
      pushBlock("link-button", renderLinkButtonHtml(linkButton));
      continue;
    }
    ...
    if (
      ...
      while (rowIndex < lines.length) {
        ...
        if (parseInlineImageToken(rowTrimmed) || parseAffiliateToken(rowTrimmed) || parseLinkButtonToken(rowTrimmed)) break;
