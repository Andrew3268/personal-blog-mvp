(function () {
  ...
  const isTouchSearchDevice = window.matchMedia('(pointer: coarse)').matches || Number(navigator.maxTouchPoints || 0) > 0;
  ...
  function syncSearchViewport() {
    ...
    const viewportOffsetTop = Math.max(0, Math.round(visualViewport ? visualViewport.offsetTop : 0));
    const visibleViewportHeight = Math.max(0, Math.round(visualViewport ? visualViewport.height : window.innerHeight));
    const topbarHeight = Math.max(0, Math.round(topbar.getBoundingClientRect().height || topbar.offsetHeight || 0));
    const availableHeight = Math.max(0, visibleViewportHeight - topbarHeight);

    /*
      position:fixed is laid out against the layout viewport while mobile keyboards
      resize/offset the visual viewport. Anchor the search surface to the live visual
      viewport, then reserve the sticky header height inside that visible area.
    */
    searchRoot.style.setProperty('--site-search-top', `${viewportOffsetTop + topbarHeight}px`);
  ...
  function openSearch() {
    ...
    searchRoot.scrollTop = 0;
    const panel = searchRoot.querySelector('.site-search__panel');
    ...
    requestAnimationFrame(() => {
      ...
      /*
        Do not autofocus on real touch devices. Mobile Safari/Chrome may pan the
        visual viewport as the software keyboard opens, which can move the search
        controls out of the visible area before the user has interacted with them.
        Desktop keeps the convenient autofocus behavior.
      */
      if (!isTouchSearchDevice) {
        window.setTimeout(() => {
          if (!isOpen) return;
